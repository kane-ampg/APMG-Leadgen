      ...
      <div className="mt-3 grid grid-cols-1 gap-1.5 rounded-lg border border-border bg-background/40 p-2.5 sm:grid-cols-2">
        {lead.phone && (
          <a
            ...
            className="tnum inline-flex items-center gap-2.5 rounded-md px-1.5 py-1.5 font-mono text-[17px] font-semibold tracking-tight text-foreground transition-colors hover:bg-muted hover:text-primary sm:col-span-2"
          >
        ...
        {lead.website && (
          <a
            ...
            data-track-lead={lead.id}
            className={cn(
              "inline-flex items-center gap-2 truncate rounded-md px-1.5 py-1 font-mono text-[12px] text-foreground transition-colors hover:bg-muted hover:text-primary",
        ...
        {!hasContact && (
          <span className="px-1.5 py-1 font-mono text-[12px] text-muted-foreground sm:col-span-2">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border pt-3">
        <span className="tnum font-mono text-[11px] text-muted-foreground">
          {lead.status === "closed_won" ? (
            ...
            <>
              Sent <span className="text-foreground">{lead.emailSentAt}</span>
            </>
          )}
        </span>
        {lead.assignedRep && (
    ...
    <div className="flex min-h-full flex-col px-4 py-5 sm:px-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-4 gap-y-2">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Every lead admin has emailed lands here, newest send first — ready to call. ·{" "}
              <span className="text-foreground/80">{SALES_REP}</span>
            </p>
          </div>
          {mode === "demo" && (
            <div className="inline-flex items-center gap-1 rounded border border-border bg-background/60 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">
              demo mode
            </div>
          )}
        </div>
      ...
      {error ? (
        <div className="mt-3 flex flex-1 flex-col items-center justify-center gap-3 rounded-xl border border-destructive/30 bg-card py-16 text-center ring-1 ring-foreground/10">
          <p className="text-sm font-medium text-foreground">Couldn&rsquo;t load the queue</p>
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3" aria-busy="true">
        ...
        <div className="mt-3 flex flex-1 items-center justify-center rounded-xl border border-dashed border-border py-16 text-center">
          <div className="max-w-md">
            <p className="text-sm font-medium text-foreground">
              {total === 0 ? "No emailed leads yet" : "No leads here"}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              {total === 0
                ? needsMigration
                  ? "The send ledger isn't set up yet — run supabase/portal-telemetry.sql, then send a campaign from the Pipeline tab."
                  : "Send a campaign from the Pipeline tab — every recipient lands here automatically."
          ...
          {pageCount > 1 && (
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-2.5 py-1 font-mono text-[11px] text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground disabled:pointer-events-none disabled:opacity-40"
              >
